...
def driver(file, other_file):
    ...
    dict1_cleaned_top = dict1_cleaned[:200]
    dict2_cleaned = sort_dictionary(collection_frequency_cleaned2)
    dict2_cleaned_top = dict2_cleaned[:200]

    # Print out requested data
    print(dict1_cleaned_top)
    print()
    print()
    print(dict2_cleaned_top)

    #wc = WordCloud(background_color="white",width=1000,height=1000, max_words=200,relative_scaling=0.5,normalize_plurals=False).generate_from_frequencies(dict1_cleaned_top)
    d1 = dict(dict1_cleaned_top)
    d2 = dict(dict2_cleaned_top)

    # YELP
    wc = WordCloud().generate_from_frequencies(d1)
    plt.imshow(wc)
    plt.show()

    # HEADLINES
    wc2 = WordCloud().generate_from_frequencies(d2)
    plt.imshow(wc2)
    plt.show()
